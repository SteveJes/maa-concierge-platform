...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.equal(response.statusCode, 200, "Expected HTTP 200 from chat route");
    ...
    assert.ok(body.conversationId, "Expected conversationId to be returned");
    ...
    assert.ok(
      ...
      "Expected callbackPersistence.requestId to be returned",
    );
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          message: "MAA callback regression passed.",
