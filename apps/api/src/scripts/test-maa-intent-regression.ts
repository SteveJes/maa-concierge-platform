...
const TEST_CASES: TestCase[] = [
  ...
  // ── Daphné fourth pass — included questions, channel consistency, multi-intent ─

  {
    id: 300,
    label: "Technogym included — must NOT return the price grid",
    userMessage: "Est-ce que l'évaluation Technogym est incluse avec l'abonnement ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [
      /Voici nos tarifs d'abonnement actuels/i,
      /\$225 par mois.*\$185.*\$195/is,
      /planifier une visite/i,
    ],
    requirePatterns: [/Technogym|évaluation|sources actuelles|valider|équipe/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 301,
    label: "Technogym included (no apostrophe) — must NOT return price grid",
    userMessage: "je veux savoir si technogym est inclus avec labonnement",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [/Voici nos tarifs d'abonnement actuels/i, /planifier une visite/i],
    requirePatterns: [/Technogym|sources actuelles|valider|équipe/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 302,
    label: "Spa/sauna access — must NOT return price grid",
    userMessage: "Est-ce que l'abonnement donne accès aux installations de spa comme sauna, vapeur ou bain remous ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [/Voici nos tarifs d'abonnement actuels/i, /planifier une visite/i],
    requirePatterns: [/sauna|vapeur|spa|bain|installations|membres|valider|conditions/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 303,
    label: "Class unlimited or per-class reservation — must NOT trigger visit CTA",
    userMessage: "Est-ce que les cours sont illimités ou faut-il réserver chaque séance ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [
      /cliquez.*bouton.*ci-dessous pour planifier votre visite/i,
      /planifier une visite/i,
    ],
    requirePatterns: [/cours|s[eé]ance|r[eé]server|conditions|équipe|varier|type/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 304,
    label: "Trainer / specialist appointment — must NOT trigger visit CTA",
    userMessage: "Comment prendre rendez-vous avec un entraîneur ou un spécialiste ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [/cliquez.*bouton.*ci-dessous pour planifier votre visite/i, /planifier une visite/i],
    requirePatterns: [/entra[iî]neur|sp[eé]cialiste|équipe|système|officiel|transmettre|confirmer/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 305,
    label: "Weight loss / fitness program — must ask clarifying questions, no visit CTA",
    userMessage: "je veux perdre du poids et avoir un programme de remise en forme. comment ca marche ?",
    locale: "fr-CA",
    forbidPatterns: [/cliquez.*bouton.*ci-dessous pour planifier votre visite/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 306,
    label: "English pricing + booking multi-intent — must answer BOTH parts in English",
    userMessage: "What are your prices and can I book in English?",
    locale: "en-CA",
    forbidPatterns: [
      /^Happy to help\. Click the button below to visit our booking page/i,
      /Bonjour|fr-CA/i,
    ],
    requirePatterns: [/\$225|\$185|\$195|\$295|month|membership|price/i],
  },
  {
    id: 307,
    label: "Informal follow-up 'c'est tu illimité ou pas' — must NOT replace with phone CTA",
    userMessage: "c'est tu illimité ou pas",
    locale: "fr-CA",
    forbidFollowUpModes: ["vapi"],
    forbidPatterns: [
      /Utilisez le bouton ci-dessous pour continuer par téléphone/i,
      /Bien sûr\. Utilisez le bouton/i,
    ],
  },
  {
    id: 308,
    label: "Restaurant menu — must include a markdown link to the official PDF",
    userMessage: "est-ce que je peux savoir vos menus cette semaine pour le resto ?",
    locale: "fr-CA",
    forbidPatterns: [/menus? .*ne sont pas publi[eé]s? en ligne/i, /pas (?:de )?menu en ligne/i],
    requirePatterns: [/\[Menu\]\(https?:\/\/|\[Petit-déjeuner\]\(https?:\/\/|clubsportifmaa\.com/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 309,
    label: "Restaurant reservation — should mention LibroReserve link or restaurant phone",
    userMessage: "j'aimerais réserver une table pour 4 personnes au restaurant",
    locale: "fr-CA",
    forbidPatterns: [/cliquez.*bouton.*ci-dessous pour planifier votre visite/i],
    requirePatterns: [/r[eé]servation|libroreserve|widgets\.libroreserve|restaurant|845-8002|R[eé]server/i],
  },

  // ── Daphné fifth pass — typos, downgrade, gym access, no-visit, clinical caution ─

  {
    id: 400,
    label: "Buanderie typo 'buandrie' — must NOT deny service exists",
    userMessage: "avez vous un service de buandrie?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [
      /je ne vois pas.*buanderie/i,
      /ne propose pas.*buanderie/i,
      /planifier une visite/i,
    ],
    requireSuppressBookingCta: true,
  },
  {
    id: 401,
    label: "Pickleball typo 'pickball' — must route to pickleball intent",
    userMessage: "Avez vous un terrain de pickball",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [
      /je ne vois pas.*pickleball/i,
      /ne propose pas.*pickleball/i,
      /planifier une visite/i,
    ],
    requireSuppressBookingCta: true,
  },
  {
    id: 402,
    label: "Pickleball misspelling 'pickelball' — must route to pickleball intent",
    userMessage: "est-ce que vous offrez du pickelball ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [/je ne vois pas.*pickleball/i, /planifier une visite/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 403,
    label: "Pickleball reservation for non-member — must NOT fall back to 'not found'",
  ...
  {
    id: 404,
    label: "Membership downgrade — must NOT 'Bien sûr' and NOT route to phone-continuation template",
    userMessage: "Je peux changer mon abonnement actuel pour un abonnement plus bas ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly", "vapi"],
    forbidPatterns: [
      /^Bien sûr\. Utilisez le bouton ci-dessous pour continuer par téléphone/i,
      /planifier une visite/i,
    ],
    requirePatterns: [/[eé]quipe|adh[eé]sions|valider|dossier|contrat|transmettre|callback/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 405,
    label: "Gym access / no booking slot — must NOT trigger visit CTA",
    userMessage: "Est-ce que les salles d'entraînement sont accessibles sans réservation ou je dois booker un créneau ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [
      /cliquez.*bouton.*ci-dessous pour planifier votre visite/i,
      /^Avec plaisir\. Cliquez/i,
    ],
    requirePatterns: [/acc[eè]s|salle|conditions|équipe|confirmer|sans réservation/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 406,
    label: "Spa with mother no membership — must NOT answer with pool/pricing",
    userMessage: "Je veux aller au spa avec ma mère, mais sans abonnement. Est-ce que c'est possible ou c'est réservé aux membres ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [
      /L'adhésion comprend l'accès à la piscine\. Les tarifs/i,
      /Voici nos tarifs/i,
    ],
    requirePatterns: [/spa|membres|non-membre|valider|équipe|conditions/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 407,
    label: "Explicit no-visit ('je veux juste m'entraîner') — must NOT trigger visit CTA",
    userMessage: "Je veux juste m'entraîner au gym, pas faire une visite. Est-ce que c'est inclus dans l'abonnement ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [/planifier une visite/i, /cliquez.*bouton.*planifier/i],
    requirePatterns: [/gym|entra[iî]nement|inclus|conditionnement|abonnement|équipe/i],
    requireSuppressBookingCta: true,
  },
  {
    id: 408,
    label: "Knee pain triage — must use cautious language, not 'idéale' or 'généralement recommandée'",
    userMessage: "Si je veux une douleur au genou évaluée, c'est physiothérapie, thérapie sportive ou entraîneur ?",
    locale: "fr-CA",
    forbidPatterns: [
      /\bidéale\b/i,
      /\bid[eé]al\b/i,
      /généralement recommandée/i,
      /est généralement recommand/i,
    ],
    requirePatterns: [/pertinent|options?|peut être|orient|équipe (?:clinique|du Club)|confirmer|confirmation/i],
  },
  {
    id: 409,
    label: "Yoga included or à la carte — must NOT confirm à la carte without source",
    userMessage: "Le yoga est inclus dans l'abonnement ou je peux payer juste un cours à la carte ?",
    locale: "fr-CA",
    forbidFollowUpModes: ["calendly"],
    forbidPatterns: [
      /\bIl est aussi possible de participer à un ou quelques cours à la carte sans être membre\b/i,
    ],
    requireSuppressBookingCta: true,
  },
];
